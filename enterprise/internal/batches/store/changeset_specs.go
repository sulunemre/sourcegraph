	...
	inserter := func(inserter *batch.Inserter) error {
		for _, c := range cs {
			...
			if err := inserter.Insert(
				...
				nullStringColumn(c.HeadRef),
				nullStringColumn(c.Title),
				nullInt32Column(int32(c.HeadRepoID)),
				nullStringColumn(c.BaseRev),
				nullStringColumn(c.BaseRef),
				nullStringColumn(c.Body),
				published,
				nullStringColumn(c.CommitMessage),
				nullStringColumn(c.CommitAuthorName),
				nullStringColumn(c.CommitAuthorEmail),
				c.Typ,
			); err != nil {
				return err
			}
