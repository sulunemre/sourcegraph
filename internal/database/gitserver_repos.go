...
type GitserverRepoStore interface {
	...
	// UpdateRepoSizes sets repo sizes according to input map. Key is repoName, value is repo_size_bytes.
	UpdateRepoSizes(ctx context.Context, shardID string, repos map[api.RepoName]int64) error
}
...
func (s *gitserverRepoStore) UpdateRepoSizes(ctx context.Context, shardID string, repos map[api.RepoName]int64) (err error) {
	// TODO: Is batch inserter the right instrument here?
	inserter := func(inserter *batch.Inserter) error {
		for repo, size := range repos {
			if err := inserter.Insert(ctx, repo, shardID, size, types.CloneStatusCloned, "NOW()"); err != nil {
				return err
			}
		}
		return nil
	...
	if err := batch.WithInserterWithReturn(
		...
		"ON CONFLICT (repo_id) DO UPDATE SET (repo_size_bytes, shard_id, clone_status, updated_at) = (EXCLUDED.repo_size_bytes, gitserver_repos.shard_id, EXCLUDED.clone_status, now())",
		nil,
		nil,
		inserter,
	); err != nil {
